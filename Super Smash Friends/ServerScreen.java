...
public class ServerScreen extends JPanel implements ActionListener, MouseListener, KeyListener, Runnable {
	...
	public ServerScreen() throws IOException {
		...
		p1Animation = new PlayerAnimation(null,"",gameRunning, damageTick);	
		p2Animation = new PlayerAnimation(null,"",gameRunning, damageTick);	
	...
	public void listen() throws IOException {
		while(true) {
			if(gameRunning) {
				try {
					p1Animation = (PlayerAnimation) in.readObject();

					if(p1Animation.isRunning() && gameRunning) {
						bothRunning = true;
					} else {
						bothRunning = false;
					}

					if(p1Animation.getPlayer() != null && p1Animation.getWeapon() != "" && playerOne != null) {
						playerOne.setAll(p1Animation.getPlayer());
						playerSword.setAll(p1Animation.getWeapon());
						playerProjectile.setAll(p1Animation.getWeapon());
					}
	...
	private void write() throws IOException{
		if(out != null) {
			out.reset();
			p2Animation = new PlayerAnimation(null,"", gameRunning, damageTick);
			if(playerTwo != null && playerTwo.getType() == 1) {
				p2Animation = new PlayerAnimation(playerTwo.animateString(), enemyProjectile.toString(),gameRunning, damageTick);
			} else if(playerTwo != null && playerTwo.getType() == 2) {
				p2Animation = new PlayerAnimation(playerTwo.animateString(), enemySword.animateString(),gameRunning, damageTick);
			}
			out.writeObject(p2Animation);
			damageTick = 0;
		}
	...
	private void playerSetup() {
		playerOne = new Player(terrainList, 225, 400, 0, 0, 0,3);
		playerTwo = new Player(terrainList, 555, 400, playerType, 0, getJumps(playerType),3);
		...
		playerTracker = new PlayerTracker(playerTwo);
	...
	public void paintComponent(Graphics g) {
		...
		if(!gameRunning) {
			...
			*/
			...
			g.drawString("*  Passive: Double Jump", 235, 475);
		...
		while(true) {
			synchronized(this) {
				if(bothRunning) {
					...
					playerTwo.setCharge(abilityCharge);
					...
					if(jumpCooldown==0 && playerTwo.onPlatform()) {
						jmCooldown = false;
						jumpCooldown = 2;
						playerTwo.resetJumps();
					}
					...
					if(left && up && playerTwo.getJumps()>=0) {
						playerTwo.move("up");
						playerTwo.move("left");
					} else if(left && down) {
						playerTwo.move("down");
						playerTwo.move("left");
					} else if(right && up && playerTwo.getJumps()>=0) {
						playerTwo.move("up");
						...
						playerTwo.move("left");
					} else if(up && playerTwo.getJumps()>=0) {
						playerTwo.move("up");
					} else if(right) {
						playerTwo.move("right");
					} else if(down) {
	...
	public void keyPressed(KeyEvent e) {
		...
		//ctrl keycode is 17 and E is 69
		if(bothRunning) {
			...
			if(e.getKeyCode() == 87 || e.getKeyCode() == 38) {
				if(!jmCooldown && playerTwo.getJumps()>0) {
					up = true;
					jmCooldown = true;
					playerTwo.setJumps((playerTwo.getJumps()-1));
				}
			}
	...
	public void keyReleased(KeyEvent e) {
		if(bothRunning) {
			if(e.getKeyCode() == 32 || e.getKeyCode() == 81) {
				if(!abCooldown) {
					if(playerTwo.getType() == 1) {
						enemyProjectile = new Projectile(terrainList, playerTwo, abilityCharge);
						projectileThreads[0] = new Thread(enemyProjectile);
						projectileThreads[0].start();
						abilityReset();
					} else if(playerTwo.getType() == 2) {
						enemySword.setCharge(abilityCharge);
						enemySword.setSwing(true);
						abilityReset();
					}
				}
			}
